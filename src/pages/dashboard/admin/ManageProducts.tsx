...
//           <button
//             key={index}
//             className={`btn mx-1 bg-yellow-300 hover:bg-yellow-400 border-none ${
...
const ManageProducts: React.FC = () => {
  ...
  if (isLoading) return <p className="text-center text-black">Loading products...</p>;

  const totalPages = Math.ceil((data?.data?.length || 0) / itemsPerPage);
  const paginatedData = data?.data?.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  return (
    <div className="container mx-auto p-4 bg-white text-black shadow-lg rounded-lg">
      <div className="flex justify-between border-b pb-4 mb-4">
        <h2 className="text-2xl font-bold">Manage Products</h2>
        <button className="bg-yellow-400 text-black font-bold py-2 px-4 rounded shadow" onClick={openAddProductModal}>
          + Add Product
        </button>
      </div>
      <div className="overflow-x-auto">
        <table className="table-auto w-full border border-gray-300">
          ...
          <tbody>
            {paginatedData?.map((product: any) => (
              <tr key={product._id} className="hover:bg-yellow-100">
                <td className="border px-4 py-2">{product.name}</td>
                <td className="border px-4 py-2">{product.brand}</td>
                <td className="border px-4 py-2">{product.type}</td>
                <td className="border px-4 py-2">${product.price}</td>
      ...
      <div className="flex justify-center mt-4 space-x-2">
        ...
        {[...Array(totalPages)].map((_, index) => (
          <button
            key={index}
            className={`px-4 py-2 rounded shadow ${currentPage === index + 1 ? "bg-yellow-500 text-black" : "bg-gray-300 text-black"}`}
            onClick={() => setCurrentPage(index + 1)}
          >
            {index + 1}
          </button>
